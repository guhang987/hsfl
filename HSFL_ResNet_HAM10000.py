...
#===================================================================
program = "HSFL ResNet18 on HAM10000"
...
net_glob_client2 = ResNet18_client_side2()
if torch.cuda.device_count() > 1:
    print("We use",torch.cuda.device_count(), "GPUs")
    net_glob_client = nn.DataParallel(net_glob_client)    
    net_glob_client2 = nn.DataParallel(net_glob_client2) 
net_glob_client.to(device)
net_glob_client2.to(device) 
print(net_glob_client)     
print(net_glob_client2)  
#=====================================================================================================
...
for iter in range(epochs):
    ...
    for idx in idxs_users[:5]:
     ...
     #后5个 6层   
    for idx in idxs_users[5:]:
        local = Client(net_glob_client2, idx, lr, device, dataset_train = dataset_train, dataset_test = dataset_test, idxs = dict_users[idx], idxs_test = dict_users_test[idx])
        # Training ------------------
        w_client = local.train(net = copy.deepcopy(net_glob_client2).to(device))
        w_locals_client.append(copy.deepcopy(w_client))
        
        # Testing -------------------
        local.evaluate(net = copy.deepcopy(net_glob_client2).to(device), ell= iter)        
    ...
    # 填充参数时，只填充前几层
    w_glob_client.pop('module.layer2.0.weight')
    w_glob_client.pop('module.layer2.1.weight')
    w_glob_client.pop('module.layer2.1.bias')
    w_glob_client.pop('module.layer2.1.running_mean')
    w_glob_client.pop('module.layer2.1.running_var')
    w_glob_client.pop('module.layer2.1.num_batches_tracked')
    w_glob_client.pop('module.layer2.3.weight')
    w_glob_client.pop('module.layer2.3.bias')
    w_glob_client.pop('module.layer2.4.weight')
    w_glob_client.pop('module.layer2.4.bias')
    w_glob_client.pop('module.layer2.4.running_mean')
    w_glob_client.pop('module.layer2.4.running_var')
    w_glob_client.pop('module.layer2.4.num_batches_tracked')
    net_glob_client2.load_state_dict(w_glob_client)    
#===================================================================================     
